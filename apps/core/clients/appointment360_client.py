...
class Appointment360Client:
    ...
    def __init__(self, graphql_client: Optional[GraphQLClient] = None):
        """
        ...
        if not self.enabled:
            logger.warning("Appointment360 GraphQL is not enabled (GRAPHQL_ENABLED=False or missing API key)")
    ...
    def logout(self, access_token: str) -> bool:
        ...
        try:
            result = self.graphql_client.execute_mutation(mutation, headers=headers, use_cache=False)
    ...
    def get_me(self, access_token: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Dictionary with user info (uuid, email, name) or None if not authenticated
        """
        if not self.enabled:
            return None
